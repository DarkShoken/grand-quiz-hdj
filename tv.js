(() => {
  ...
  const room = G.cleanRoom(G.qs('room', 'QUIZ01'));
  G.setRoomInUrl(room);
  ...
  function renderLobby(){
    ...
    document.getElementById('playerList').innerHTML = players.length ? players.map(p => `<div class="player-chip"><strong>${G.escapeHtml(p.name)}</strong>${currentState?.mode === 'teams' && p.team ? `<span class="badge">Équipe ${G.escapeHtml(p.team)}</span>` : ''}</div>`).join('') : '<div class="muted">En attente des joueurs…</div>';
  }
  function render(){
    ...
    document.getElementById('answerCount').textContent = `${answers} réponse${answers > 1 ? 's' : ''}`;
  ...
  function renderQuestion(){
    ...
    } else if (q.type === 'numeric') body = `<div class="reveal-answer" style="color:var(--cyan)">🔢 Entrez votre réponse sur votre téléphone</div>`;
    else body = `<div class="reveal-answer pulse" style="color:var(--red)">🚨 BUZZEZ !</div><div class="explanation">${currentState.buzzedPlayer ? `Le plus rapide : <strong>${G.escapeHtml(currentState.buzzedPlayer)}</strong>` : 'Le premier qui buzze obtient la main.'}</div>`;
  ...
  function startTimer(){
    const update = () => {
      ...
      if (tv) tv.textContent = sec; if (bar) bar.style.width = `${Math.max(0, Math.min(100, left / duration * 100))}%`;
    }; update(); timer = setInterval(update, 200);
  }
  function renderReveal(){
    const q = currentState.question; const correct = currentState.correctLabel || '';
    let options = '';
    if ((q.type === 'mcq' || q.type === 'truefalse') && q.options) options = `<div class="answer-grid">${q.options.map((opt,i)=>`<div class="answer-tile ${['a','b','c','d'][i] || 'a'} ${i === currentState.correctIndex ? 'correct' : ''}">${String.fromCharCode(65+i)} · ${G.escapeHtml(opt)}</div>`).join('')}</div>`;
