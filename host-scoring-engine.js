(() => {
  ...
  function acceptedAnswers(questionId, expected) {
    const map = window.GRAND_QUIZ_ACCEPTED_ANSWERS;
    const extras = map instanceof Map ? (map.get(questionId) || []) : [];
    return [expected, ...extras]
      .filter(Boolean)
      .filter((value, index, values) => values.findIndex((other) => normalizeText(other) === normalizeText(value)) === index);
  }
  ...
  function publicPlayers() {
    ...
    return [...playersMap.values()].map(({ id, name, team, score, online }) => ({ id, name, team, score: Number(score) || 0, online }));
  }
  ...
  function ranking(mode) {
    ...
    return [...playersMap.values()].map((player) => ({ id: player.id, name: player.name, score: Number(player.score) || 0 })).sort((a, b) => b.score - a.score);
  }

  function installEditableAnswers() {
    if (!playersMap || !answersMap) return;
    Object.defineProperty(answersMap, 'has', { configurable: true, value() { return false; } });
    Object.defineProperty(answersMap, 'set', {
      ...
      value(playerId, nextAnswer) {
        ...
        if (previous && player) player.score = Math.max(0, (Number(player.score) || 0) - (Number(previous.points) || 0));
        if (nextAnswer && typeof nextAnswer === 'object') nextAnswer.revisions = previous ? (Number(previous.revisions) || 0) + 1 : 0;
  ...
  function scoreFreeText(payload) {
    ...
    const allowedAnswers = acceptedAnswers(questionId, expected);
    ...
    if (lastTextQuestionId !== questionId || !lastTextResults) {
      ...
      for (const [id, player] of playersMap.entries()) {
        ...
        const correct = Boolean(answer) && allowedAnswers.some((candidate) => textAnswerIsCorrect(answer.value, candidate));
  ...
  function syncPublicAnswerState(payload) {
    ...
    payload.playerAnswers = Object.fromEntries([...answersMap.entries()].map(([playerId, answer]) => [playerId, answer?.value]));
