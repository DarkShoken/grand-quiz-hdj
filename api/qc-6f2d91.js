...
module.exports=async function handler(req,res){
 ...
 try{
  const sr=await fetch(`${SUPABASE_URL}/rest/v1/rpc/gq26_calibration_batch`,{method:'POST',headers:{apikey:SUPABASE_ANON_KEY,Authorization:`Bearer ${SUPABASE_ANON_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({p_offset:offset,p_limit:limit})});
  const items=await sr.json().catch(()=>[]);if(!sr.ok||!Array.isArray(items))return res.status(502).json({error:'batch read failed',status:sr.status});
  if(!items.length)return res.status(200).json({model:MODEL,offset,count:0,results:[]});
  ...
  const gr=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${MODEL}:generateContent`,{method:'POST',headers:{'x-goog-api-key':process.env.GEMINI_API_KEY,'Content-Type':'application/json'},body:JSON.stringify({contents:[{role:'user',parts:[{text:prompt}]}],generationConfig:{temperature:0,topP:0.2,maxOutputTokens:6500,responseMimeType:'application/json',responseSchema:schema}})});
  const data=await gr.json().catch(()=>({}));if(!gr.ok)return res.status(gr.status===429?429:502).json({error:data?.error?.message||'Gemini error',model:MODEL});
  const raw=outputText(data);if(!raw)return res.status(502).json({error:'empty model output',model:MODEL});const parsed=JSON.parse(raw);const byId=new Map((parsed.results||[]).map(x=>[String(x.id),Math.max(0,Math.min(100,Math.round(Number(x.successRate))))]));
  const results=qs.map(q=>({id:q.id,successRate:byId.has(q.id)?byId.get(q.id):null}));return res.status(200).json({model:MODEL,offset,count:results.length,results});
 }catch(e){console.error(e);return res.status(500).json({error:'calibration failed',model:MODEL})}
};
