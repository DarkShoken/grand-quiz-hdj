(() => {
  ...
  const textTypes = new Set(['free', 'text', 'open', 'open_text', 'freeform', 'written']);
  ...
  function questionType() {
    return String(currentState?.question?.type || '').toLowerCase();
  }

  function isFreeTextType(type = questionType()) {
    return textTypes.has(type) || !['mcq', 'truefalse', 'numeric', 'buzzer'].includes(type);
  }

  function statusLabel() {
    if (answerNotice) return answerNotice;
    if (questionType() === 'buzzer' && currentState?.buzzedPlayerId === playerId) return '🚨 À toi !';
  ...
  function syncQuestionDom() {
    ...
    if (freeButton) freeButton.textContent = hasSubmitted() ? 'Modifier ma réponse' : 'Envoyer ma réponse';

    const buzzButton = document.getElementById('buzzBtn');
    const buzzStatus = document.querySelector('.buzzer-status');
    if (buzzButton) {
      const activePlayerId = currentState.buzzedPlayerId || null;
      const locked = Boolean(activePlayerId);
      buzzButton.disabled = locked;
      buzzButton.textContent = activePlayerId === playerId ? 'TU AS BUZZÉ !' : locked ? 'TROP TARD' : 'BUZZ !';
      if (buzzStatus) {
        const text = activePlayerId
          ? activePlayerId === playerId
            ? 'Tu as pris la main. Donne ta réponse au soignant.'
            : `${currentState.buzzedPlayer || 'Un participant'} a pris la main.`
          : 'Appuie dès que tu connais la réponse.';
        if (buzzStatus.textContent !== text) buzzStatus.textContent = text;
      }
    }
    ...
    if (message.type === 'state') {
      ...
      if (nextQuestionId && hasOwnAnswer(currentState, playerId)) {
        ...
        if (!localSelectionIsFresh) {
          ...
          if (questionType() === 'numeric' || isFreeTextType()) answerDraftValue = storedValue;
        }
  ...
  function sendBuzz() {
    const questionId = currentState?.question?.id;
    if (!questionId || currentState?.phase !== 'question' || currentState?.buzzedPlayerId) return;
    transport.send('buzz', { playerId, name, team, questionId });
  }
  ...
  function renderQuestion() {
    ...
    const previousInput = document.getElementById('numericInput') || document.getElementById('freeInput');
    const wasFocused = document.activeElement === previousInput;
    const start = wasFocused ? previousInput.selectionStart : null;
    const end = wasFocused ? previousInput.selectionEnd : null;
    if (previousInput && answerDraftQuestionId === question.id) answerDraftValue = previousInput.value;
    ...
    if (type === 'mcq' || type === 'truefalse') {
      controls = `<div class="mobile-options">${question.options.map((option, index) => {
        const value = type === 'truefalse' ? (index === 0 ? 'true' : 'false') : index;
        return `<button class="mobile-option ${isSelected(value) ? 'chosen' : ''}" data-value="${value}">${G.escapeHtml(option)}</button>`;
      }).join('')}</div>`;
    } else if (type === 'numeric') {
      controls = `<div class="direct-answer-panel"><label for="numericInput">Ta réponse</label><div class="numeric-row"><input id="numericInput" inputmode="decimal" type="text" value="${G.escapeHtml(answerDraftValue)}" placeholder="Entre un nombre" autocomplete="off"><button id="numericBtn" class="btn green">${hasSubmitted() ? 'Modifier' : 'Envoyer'}</button></div></div>`;
    } else if (type === 'buzzer') {
      const activePlayerId = currentState.buzzedPlayerId || null;
      const buttonText = activePlayerId === playerId ? 'TU AS BUZZÉ !' : activePlayerId ? 'TROP TARD' : 'BUZZ !';
      const feedback = activePlayerId
        ? activePlayerId === playerId
          ? 'Tu as pris la main. Donne ta réponse au soignant.'
          : `${currentState.buzzedPlayer || 'Un participant'} a pris la main.`
        : 'Appuie dès que tu connais la réponse.';
      controls = `<div class="buzzer-mobile-panel"><button id="buzzBtn" class="big-buzzer" ${activePlayerId ? 'disabled' : ''}>${buttonText}</button><div class="feedback buzzer-status">${G.escapeHtml(feedback)}</div></div>`;
    } else {
      controls = `<div class="direct-answer-panel"><label for="freeInput">Ta réponse</label><input id="freeInput" maxlength="160" type="text" value="${G.escapeHtml(answerDraftValue)}" placeholder="Écris ta réponse…" autocomplete="off" autocapitalize="sentences"><button id="freeBtn" class="btn green">${hasSubmitted() ? 'Modifier ma réponse' : 'Envoyer ma réponse'}</button></div>`;
    }
    ...
    const input = document.getElementById('numericInput') || document.getElementById('freeInput');
    ...
    const submitDirect = () => {
      if (!input) return;
      answerDraftValue = input.value;
      const value = answerDraftValue.replace(/\s+/g, ' ').trim();
      if (!value) {
        input.focus();
        return;
      }
      sendAnswer(type === 'numeric' ? value.replace(',', '.') : value);
    };

    document.getElementById('numericBtn')?.addEventListener('click', submitDirect);
    document.getElementById('freeBtn')?.addEventListener('click', submitDirect);
